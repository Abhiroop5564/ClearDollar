...
export function BudgetPage() {
    ...
    const TreeRow = React.forwardRef<HTMLDivElement, TreeItemComponentProps<TagTreeNode>>(
        (props, ref) => {
            ...
            useEffect(() => {
                const nextDisplay =
                    mode === "expenses" ? Math.abs(item.budgetAmount) : item.budgetAmount;
                setAmountDraft(String(nextDisplay));
            }, [item.budgetAmount, mode]);
                            ...
                            <input
                                ...
                                onKeyDown={(e) => {
                                    ...
                                    if (e.key === "Enter") {
                                        ...
                                        setSaveStatus(null);
                                        setActiveTree((prev) =>
                                            updateNodeName(prev, item.tagId, trimmed)
                                        );
                            ...
                            <input
                                ...
                                onKeyDown={(e) => {
                                    ...
                                    if (e.key === "Enter") {
                                        ...
                                        const committed = Math.abs(raw);

                                        setSaveStatus(null);
                                        setActiveTree((prev) =>
                                            updateNodeBudget(prev, item.tagId, committed)
                                        );
                ...
                <div className="bg-white border rounded-2xl p-4">
                    <div className="flex flex-col gap-3">
                        <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                            <div>
                                <div className="text-lg font-semibold">Tag Tree Editor</div>
                                <div className="text-sm text-gray-500">
                                    Drag and drop to reorder or change parent. Save when you're done.
                                </div>
                            </div>
                    ...
                    <div className="bg-white border rounded-2xl p-4">
                        <SortableTree<TagTreeNode>
                            ...
                            onItemsChanged={(newItems) => {
                                setSaveStatus(null);
                                setActiveTree(syncParentIds(newItems));
                            }}
    ...
    async function saveChanges() {
        ...
        try {
            const payload: TagDto[] = [
                ...flattenForSave(incomeTree),
                ...flattenForSave(expensesTree),
            ];
            ...
            if (!res.ok) {
                const text = await res.text().catch(() => "");
                throw new Error(text ? `Save failed: ${res.status} ${text}` : `Save failed: ${res.status}`);
            }

            setSaveStatus({ kind: "ok", message: "Saved successfully." });
        } catch (err: any) {
            setSaveStatus({ kind: "err", message: err?.message ?? "Save failed (unknown error)." });
        } finally {
            setIsSaving(false);
        }
    }
